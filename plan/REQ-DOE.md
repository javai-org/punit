...
>
> Note: `@FactorSetter`, `@FactorGetter`, and `registerAutoWired` are deprecated as of 0.6.0 and scheduled for removal. DoE integration must use the immutable use case pattern exclusively — design matrices produce `FactorArguments` streams consumed by `registerWithFactors` factories, not setter-based injection.
