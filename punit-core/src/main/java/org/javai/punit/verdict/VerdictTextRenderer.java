...
import static org.javai.punit.statistics.transparent.StatisticalVocabulary.P_HAT;
...
public final class VerdictTextRenderer {
	...
	private static final Map<String, String> TOOLTIPS = Map.ofEntries(
			...
			Map.entry("SE(" + P_HAT + "):", "Standard error of the observed proportion — measures sampling noise in " + P_HAT),
			Map.entry("CI lower bound:", "Wilson score lower bound — we are this confident the true rate is at least this value"),
			Map.entry("Z:", "How many standard errors " + P_HAT + " is from the threshold \u03C0\u2080 — negative means below"),
	...
	public static String renderSummary(ProbabilisticTestVerdict verdict) {
		...
		if (exec.warmup() > 0) {
			sb.append(PUnitReporter.labelValueLn("Warmup:", String.format("%d discarded invocations", exec.warmup())));
		}
	...
	public static String renderStatisticalAnalysis(ProbabilisticTestVerdict verdict) {
		...
		sb.append(PUnitReporter.labelValueLn("Confidence level:", String.format("%.1f%%", stats.confidenceLevel() * 100)));
		sb.append(PUnitReporter.labelValueLn("SE(" + P_HAT + "):", String.format(" %.4f", stats.standardError())));
		sb.append(PUnitReporter.labelValueLn("CI lower bound:", String.format("%.4f", stats.ciLower())));
	...
	private void renderObservedDataSection(StringBuilder sb, ProbabilisticTestVerdict verdict) {
		...
		sb.append(statLabel("Observed rate (" + P_HAT + "):",
				RateFormat.format(exec.observedPassRate())));
		if (exec.warmup() > 0) {
			String useCaseLabel = verdict.identity().useCaseId().orElse("");
			sb.append(statLabel("Warmup:", String.format("%d discarded invocations", exec.warmup())));
		}
		sb.append("\n");
	}
	...
	private void renderInferenceSection(StringBuilder sb, ProbabilisticTestVerdict verdict) {
		...
		// SE with formula
		sb.append(statLabel("SE(" + P_HAT + "):",
				String.format("SE = %s(%s(1-%s)/n) = %s(%.2f %s %.2f / %d) = %.4f",
						symbols.sqrt(), P_HAT, P_HAT,
	...
	private void renderZTestCalculation(StringBuilder sb, ProbabilisticTestVerdict verdict) {
		...
		sb.append(statLabel("Z:",
				String.format("z = (%s - %s%s) / %s(%s%s(1-%s%s)/n)",
						P_HAT, symbols.pi(), StatisticalVocabulary.SUB_ZERO,
